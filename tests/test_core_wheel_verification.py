"""Contract tests for exact known-release PDS Core wheel authentication."""
...
def _wheel(
    ...
    *,
    ...
    selected_filename = filename or contract.filename
    selected_distribution = distribution or contract.distribution
    selected_version = version or contract.version
    path = root / selected_filename
    dist_info_version = selected_version.replace("-", "_")
    with zipfile.ZipFile(path, "w") as archive:
        archive.writestr("pds_core/__init__.py", "")
        if include_metadata:
            archive.writestr(
                f"pds_core-{dist_info_version}.dist-info/METADATA",
                (
                    "Metadata-Version: 2.4\n"
                    f"Name: {selected_distribution}\n"
...
def _matching_contract(
    path: Path,
    base: CoreWheelContract = CoreWheelContract(),
) -> CoreWheelContract:
...
def test_correct_metadata_and_hash_contract_is_accepted(
    ...
    contract = known_core_wheel_contract(version)
    wheel = _wheel(tmp_path, contract=contract)
    verified = verify_core_wheel(wheel, _matching_contract(wheel, contract))
    assert verified.filename == contract.filename
...
def test_063_wheel_cannot_authenticate_against_060_contract(tmp_path: Path) -> None:
    current = known_core_wheel_contract("0.6.3")
    wheel = _wheel(tmp_path, contract=current)
    with pytest.raises(CoreWheelVerificationError, match="filename"):
        verify_core_wheel(wheel, known_core_wheel_contract("0.6.0"))
...
def _installed_identity(
    ...
    *,
    ...
    monkeypatch.setitem(
        sys.modules,
        "pds_core",
        SimpleNamespace(__file__=str(import_path)),
    )
...
def test_installed_identity_accepts_explicit_selected_release(
    ...
    result = installed_core_identity(known_core_wheel_contract(version))
...
def test_installed_metadata_version_mismatch_is_rejected(
    ...
    with pytest.raises(CoreWheelVerificationError, match="version"):
        installed_core_identity(known_core_wheel_contract("0.6.0"))
