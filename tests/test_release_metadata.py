...
def test_active_release_documents_name_the_candidate_version() -> None:
    for relative in (
        ...
        "docs/v0.10.3_installed_resubmission_inbox_acceptance.md",
    ):
        assert "0.10.3" in (ROOT / relative).read_text(encoding="utf-8"), relative


def test_prior_release_evidence_remains_historical() -> None:
    required = (
        ("docs/v0.10.2_installed_selected_review_read_acceptance.md", "0.10.2"),
