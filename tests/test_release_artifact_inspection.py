    ...
    *,
    ...
    with zipfile.ZipFile(path, "w") as archive:
        for required in (
            ...
            "submission_evidence_resolution.py",
            "submission_evidence_validation.py",
            "cli_app/handlers/resubmission_inbox.py",
        ):
            archive.writestr(f"quillan/{required}", "")
        archive.writestr("quillan/_version.py", '__version__ = "0.10.3"\n')
        archive.writestr(extra_name, "")
        archive.writestr("quillan-0.10.3.dist-info/METADATA", metadata)
        archive.writestr(
            "quillan-0.10.3.dist-info/entry_points.txt",
        ...
        archive.writestr("quillan-0.10.3.dist-info/licenses/LICENSE", "MIT\n")
    return path
...
def _sdist(
    ...
    *,
    metadata: str = VALID_METADATA,
    package_name: str = "quillan-0.10.3",
) -> Path:
    ...
    for required in (
        ...
        required_path = package / "quillan" / required
...
def test_ordinary_current_package_paths_are_accepted(tmp_path: Path) -> None:
    assert inspect_wheel(_wheel(tmp_path / "current.whl"))["version"] == "0.10.3"
    assert inspect_sdist(_sdist(tmp_path / "current.tar.gz"))["version"] == "0.10.3"
...
def test_sdist_requires_exact_release_root(tmp_path: Path) -> None:
    artifact = _sdist(
        ...
        package_name="not-quillan-0.10.3",
    )
    with pytest.raises(AssertionError):
        inspect_sdist(artifact)
